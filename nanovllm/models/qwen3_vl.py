...
class Qwen3VLForConditionalGeneration(nn.Module):
    ...
    def forward(
        ...
        if vision_slices_per_seq:
            ...
            # Compare with normal path: normal path uses vision_slices_per_seq with target_offset
                ...
                for seq_idx, (start, end) in enumerate(zip(offsets[:-1], offsets[1:])):
                    ...
                    if seq_image_chunks:
                        ...
                        seq_vision_tokens = torch.cat([
                            chunk.to(inputs_embeds.device, inputs_embeds.dtype)
                            for chunk in seq_image_chunks
                        ], dim=0)
                        ...
                        if seq_vision_placeholders and seq_idx < len(seq_vision_placeholders):
                            ...
                            if placeholders:
                                ...
                                if len(placeholders) != len(seq_image_chunks):
                                    raise ValueError(
                                        f"Sequence {seq_idx}: {len(placeholders)} "
                                        f"placeholder range(s) do not match "
                                        f"{len(seq_image_chunks)} image chunk(s)"
                                    )
                                for (target_offset, expected_length), chunk in zip(
                                    placeholders, seq_image_chunks
                                ):
                                    chunk_tokens = chunk.to(
                                    ...
                                    if expected_length != chunk_len:
                                        raise ValueError(
                                            ...
                                            f"match image chunk length {chunk_len}"
                                        )
                                    target_start = start + target_offset
                        ...
                        if deepstack_layers_raw:
                            if deepstack_collect is None:
                                deepstack_collect = [[] for _ in range(len(deepstack_layers_raw))]
                            for layer_idx in range(len(deepstack_layers_raw)):
                                if img_start_idx < len(deepstack_layers_raw[layer_idx]):
                                    layer_chunks = deepstack_layers_raw[layer_idx][img_start_idx:img_end_idx]
                                    if layer_chunks:
