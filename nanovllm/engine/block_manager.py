...
class BlockManager:
    ...
    def can_allocate(self, seq: Sequence) -> bool:
        # If no KV cache blocks are available (prefill-only mode without KV cache),
        # we can always allocate since we don't need to allocate blocks
    ...
    def can_append(self, seq: Sequence) -> bool:
        # If no KV cache blocks are available (prefill-only mode without KV cache),
        # we can't append (but this shouldn't be called in prefill-only mode)
        if len(self.blocks) == 0:
            return False
